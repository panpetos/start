...
try {
    if ($action === 'ice') {
        ...
        // ── ПОЧЕМУ СПИСОК РАЗНОРОДНЫЙ, А НЕ ОДИН GOOGLE ────────────────────────
        // Раньше здесь стояли четыре адреса, три из которых — Google. И это,
        // похоже, и было причиной жалоб «звонок идёт только с VPN».
        //
        // STUN нужен браузеру, чтобы узнать свой внешний адрес. Если STUN
        // недоступен, внешний адрес неизвестен, кандидатов типа srflx нет —
        // и соединение не встаёт. В России сервисы Google регулярно режут, то
        // есть у части людей stun.l.google.com просто не отвечает. Включают VPN —
        // Google становится доступен, и звонок внезапно «чинится». Симптом
        // выглядит как проблема с NAT, а на деле недоступен сам STUN.
        //
        // Поэтому список собран из разных операторов и юрисдикций: первым —
        // российский (доступен без VPN), затем мировые. Хватит любого одного.
        // Больше пяти не ставим: каждый добавляет кандидатов и замедляет дозвон.
        // Все адреса проверены на существование — мёртвых в списке нет.
        $servers = [['urls' => [
            'stun:stun.sipnet.ru:3478',        // Россия — работает там, где Google режут
            'stun:stun.cloudflare.com:3478',   // мировой, обычно не блокируется
            'stun:stun.l.google.com:19302',    // мировой, IPv6
            'stun:stun.nextcloud.com:3478',    // ЕС, IPv6
            'stun:stun.antisip.com:3478',      // ЕС, IPv6
        ]]];
