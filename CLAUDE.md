...
## Платежи и выплаты — ответ Робокассы (сентябрь 2026, зафиксировать)
Агентская схема через Робокассу **возможна только с «Robokassa Онлайн»** (своя облачная касса на юрлицо/ИП, аренда от 1600 ₽/мес при оплате за год; комиссия сверху — карты РФ 3,9%, СБП 2,7% на «Стартовом» тарифе). Только такая касса даёт в чеке признак агента и данные принципала-психолога. **«Робочеки» для агентской схемы НЕ подходят** (чек от имени Robokassa). **Чек НПД за самозанятого Робокасса не формирует** — психолог сам пробивает чек в «Мой налог» после выплаты. **Массовых выплат на карты физлиц у Робокассы нет** (и сплит с самозанятыми не работает) — выплаты исполнителям делаются напрямую с расчётного счёта вне системы. Ограничений по обороту/сроку ИП на старте нет. Для активации магазина сайт должен быть рабочим, с оффертой/правилами оплаты и возврата и реквизитами (ИНН/ОГРН/контакты) — см. robokassa.com/how-works. Вывод: приём оплат — «Robokassa Онлайн» (касса-агент), выплаты — вручную с р/с, чеки НПД — на стороне психолога; наш учёт выплат (api/payouts.php + проверка НПД) под это уже готов.

## SEO-страница поста
Пост открывается отдельной страницей **`/post.php?id=<id>`** (серверный рендер: мета-теги, Open Graph, JSON-LD Article, тело через компактный PHP-аналог formatPostText). Лента и чат ведут на неё. Скрытый/удалённый пост → 404+noindex.
