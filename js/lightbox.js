...
(function (global) {
    ...
    function bindZoom(el) {
        ...
        const up = function (e) {
            // Отпускание слушаем на окне (палец во время жеста уходит за край картинки),
            // поэтому сюда долетают и нажатия по кнопкам — ⋮, ✕, стрелкам. Их касание
            // не начиналось на сцене, в pts его нет, и дальше шёл разбор жеста по
            // координатам ПРОШЛОГО касания: получался «свайп» и фото перелистывалось
            // само. Отсюда и «кнопка трёх точек нажимается насквозь».
            if (!pts.has(e.pointerId)) return;
